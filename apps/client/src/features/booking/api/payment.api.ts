...
export function getPublicCheckoutSessionSummary(sessionId: string) {
    return api<PaymentSessionSummaryDto>(`/api/api/v1/public/payments/checkout-session/${sessionId}`, {
        companyId: null,
    });
}

export function getPrivateCheckoutSessionSummary(sessionId: string) {
    return api<PaymentSessionSummaryDto>(`/api/api/v1/payments/checkout-session/${sessionId}`);
}