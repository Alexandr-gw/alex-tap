...
export class ScheduleService {
        ...
        return this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
            ...
            if (!clientId) {
                if (!input.dto.client?.name) {
                    throw new BadRequestException('Client name is required (or provide clientId)');
                }
    ...
    async reviewJob(input: { companyId: string; userSub: string | null; jobId: string; dto: ReviewJobDto }) {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const targetEnd = nextEnd ?? (nextStart ? addMinutes(targetStart, defaultDurationMins) : job.endAt);
