...
export class JobsService {
    ...
    async updateJob(input: {
        ...
        const updatedJob = await this.prisma.$transaction(async (tx) => {
            ...
            const workerIdsChanged =
                nextWorkerIds !== null && !this.areStringArraysEqual(existingWorkerIds, nextWorkerIds);
    ...
    async requestPaymentLink(input: {
        ...
        if (job.status === JobStatus.CANCELED) {
            throw new BadRequestException('Cannot request payment for a canceled job');
        }
    ...
    private async createManagerJob(input: {
        ...
        const targetWorkerIds =
            (await this.resolveNextWorkerIds(this.prisma, input.companyId, input.dto.workerIds, input.dto.workerId))
            ?? [];
    ...
    private async resolveAccess(companyId: string, roles: string[], userSub: string | null): Promise<AccessContext> {
        ...
        const isManager = Boolean(
            isManagerRole && membership && (membership.role === Role.ADMIN || membership.role === Role.MANAGER),
        );
    ...
    private normalizeLineItems(items: Array<{ name: string; quantity: number; unitPriceCents: number }>) {
    ...
    private resolveJobEnd(start: Date, endValue: string | undefined, serviceDurationMins: number | null) {
        if (endValue) {
    ...
    private resolveCreateLineItems(
        ...
        if (!service) {
            throw new BadRequestException('Provide a service or at least one line item');
        }
    ...
    private resolveClientName(name?: string | null, firstName?: string | null, lastName?: string | null) {
