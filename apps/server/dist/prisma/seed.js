...
async function main() {
    ...
    for (const demoUser of DEMO_USERS) {
        ...
        const membership = await prisma.membership.create({
            data: {
                ...
                userId: user.id,
                role: demoUser.role,
                lastSeenPaidJobsAt: demoUser.role === "ADMIN"
                    ? addDays(today, -1)
    ...
    const workerSeeds = DEMO_USERS.filter((user) => user.role === "WORKER");
    const workers = [];
    ...
    async function createJobSeed(params) {
        const lineItems = [params.primaryService, params.secondaryService].filter((service) => Boolean(service));
        ...
        const endAt = addMinutes(params.startAt, lineItems.reduce((total, service) => total + service.durationMins, 0));
        const assignedWorkers = [params.worker, params.secondaryWorker].filter((worker) => Boolean(worker));
        const createdAt = params.startAt < now ? addDays(params.startAt, -4) : addDays(params.startAt, -6);
        const job = await prisma.job.create({
            data: {
                ...
                paidAt: params.status === client_1.JobStatus.DONE ? addMinutes(endAt, 45) : null,
                createdAt,
                lineItems: {
        ...
        if (jobSequence % 4 === 0 || params.status === client_1.JobStatus.PENDING_CONFIRMATION) {
            await prisma.jobComment.create({
                data: {
                    jobId: job.id,
                    authorUserId: ensuredManagerUser.id,
                    message: params.status === client_1.JobStatus.PENDING_CONFIRMATION
        ...
        if (params.status === client_1.JobStatus.DONE) {
            ...
            await prisma.payment.create({
                data: {
                    companyId: company.id,
                    jobId: job.id,
                    provider: useCash ? client_1.PaymentProvider.CASH : client_1.PaymentProvider.STRIPE,
        ...
        else if (params.status === client_1.JobStatus.SCHEDULED && jobSequence % 9 === 0) {
            await prisma.payment.create({
                data: {
                    companyId: company.id,
                    jobId: job.id,
        ...
        if (params.status === client_1.JobStatus.PENDING_CONFIRMATION) {
        ...
        if (params.status === client_1.JobStatus.SCHEDULED || params.status === client_1.JobStatus.IN_PROGRESS) {
            const reminderTime = addDays(params.startAt, -1);
            await prisma.notification.create({
                data: {
                    companyId: company.id,
                    type: "job_reminder",
                    channel: params.client.kind === "commercial"
        ...
        await prisma.activity.create({
            data: {
                ...
                jobId: job.id,
                clientId: params.client.id,
                actorType: params.source === "public-booking"
                    ? client_1.ActivityActorType.PUBLIC
                    : client_1.ActivityActorType.USER,
        ...
        if (params.status === client_1.JobStatus.CANCELED || params.status === client_1.JobStatus.NO_SHOW) {
            await prisma.activity.create({
                data: {
                    companyId: company.id,
                    ...
                    actorId: ensuredManagerUser.id,
                    actorLabel: "Mina Manager",
                    message: params.status === client_1.JobStatus.NO_SHOW
    ...
    for (let index = 0; index < 18; index += 1) {
        ...
        const secondaryService = index % 5 === 0 && tradeServices.length > 1 ? pick(tradeServices, index + 1) : undefined;
        const tradeWorkers = workersByTrade.get(trade) ?? workers;
        const worker = pick(tradeWorkers, index);
        const secondaryWorker = primaryService.durationMins >= 180 && tradeWorkers.length > 1
            ? pick(tradeWorkers, index + 1)
            : undefined;
        const client = pick(clients, index);
        const startAt = setTime(addDays(today, -(index + 1) * 3), 8 + ((index % 3) * 2));
        const status = index % 7 === 0
            ? client_1.JobStatus.CANCELED
            : index % 11 === 0
                ? client_1.JobStatus.NO_SHOW
                : client_1.JobStatus.DONE;
        ...
        await createJobSeed({
            ...
            status,
            source,
            note: status === client_1.JobStatus.DONE
    ...
    for (let dayOffset = 0; dayOffset < 90; dayOffset += 1) {
        ...
        for (let slot = 0; slot < jobsForDay; slot += 1) {
            ...
            const tradeServices = servicesByTrade.get(trade) ?? services;
            const primaryService = pick(tradeServices, index);
            const secondaryService = (index % 6 === 0 || primaryService.durationMins >= 210) && tradeServices.length > 1
                ? pick(tradeServices, index + 1)
                : undefined;
            const tradeWorkers = workersByTrade.get(trade) ?? workers;
            const worker = pick(tradeWorkers, index);
            const secondaryWorker = (primaryService.durationMins >= 180 || index % 8 === 0) && tradeWorkers.length > 1
                ? pick(tradeWorkers, index + 1)
                : undefined;
            const client = pick(clients, index + 8);
            let startAt = setTime(date, slot === 0 ? 8 + (index % 3) : 13 + (index % 2));
            let status = client_1.JobStatus.SCHEDULED;
            ...
            const source = status === client_1.JobStatus.PENDING_CONFIRMATION || index % 5 === 0
                ? "public-booking"
                : "admin-panel";
            await createJobSeed({
                client,
    ...
    console.log(`Seed complete: ${clients.length} clients, ${services.length} services, ${workers.length} workers, ${seededJobs.length} jobs.`);
}
main()
    .catch(error => {
    console.error(error);
